...
export class UpdateIngredientTag {
    ...
    static async execute(input: UpdateIngredientTagInput): Promise<string> {
        const ingredient: Ingredient = await this.ingredientRepository.findById(input.ingredientId);
        if (!ingredient) throw new ValidationError("Ingredient not found");

        ingredient.tag = input.newTag;
        await this.ingredientRepository.save(ingredient);

        const relatedRecipes: Recipe[] = await this.recipeRepository.findByIngredientId(input.ingredientId);

        const invalidRecipes = new Set<Recipe>();

        if (input.newTag === IngredientTag.PROTEINE && relatedRecipes.length > 1) {
            relatedRecipes.forEach(recipe => invalidRecipes.add(recipe));
        }
        ...
        const names = [...invalidRecipes].map(recipe => recipe.name).join(", ");
        return `Tag updated, but these recipes are no longer valid: ${names}`;
    }
}

function isRecipeValid(recipe: Recipe): boolean {
    const proteines: Ingredient[] = recipe.ingredients.filter(ingredient => ingredient.tag === IngredientTag.PROTEINE);
    const feculent: Ingredient[] = recipe.ingredients.filter(ingredient => ingredient.tag === IngredientTag.FECULENT);
