...
export class CreateRecipe {
    ...
    static async execute(input: CreateRecipeInput): Promise<Recipe> {
        ...
        const countCriteria: CountCriteria = getCountCriteria(ingredients);
        ...
        const proteinIds: number[] = ingredients
            .filter(ingredient => ingredient.tag === IngredientTag.PROTEINE)
...
function getCountCriteria(ingredients: Ingredient[]): CountCriteria {
    return {
        proteine: ingredients.filter((ingredient) => ingredient.tag === IngredientTag.PROTEINE).length <= 1,
        feculent: ingredients.filter((ingredient) => ingredient.tag === IngredientTag.FECULENT).length === 1,
    };
}
